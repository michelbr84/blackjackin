    ...
    render: function () {
        ...
        return (
            <div className='hand'>
                ...
                <React.addons.CSSTransitionGroup transitionName="deal-card" component="div" style={{ display: 'flex', gap: '15px' }}>
...
var Outcome = React.createClass({
    ...
    render: function () {
        if (this.props.status === 'playing' || this.props.status === 'new') {
            return null; // Don't show anything while playing
        }
        var message = (this.props.status === 'win') ? "YOU WIN" : "YOU LOSE";
        var cssClass = (this.props.status === 'win') ? "outcome-overlay outcome-win" : "outcome-overlay outcome-lose";
        ...
        return (
            <div className='interface'>
                ...
                <div className="score-board">
                    <div className="score-badge">Dealer: {this.props.dealerscore}</div>
                    <div className="score-badge">Player: {this.props.playerscore}</div>
                </div>

                <div className="controls">
                    <button onClick={this.props.deal} type="button" className="btn-action btn-deal">Deal</button>
                    <button onClick={this.props.hit} disabled={this.props.status !== 'playing'} type="button" className="btn-action btn-hit">Hit</button>
                    <button onClick={this.props.stand} disabled={this.props.status !== 'playing'} type="button" className="btn-action btn-stand">Stand</button>
                </div>
